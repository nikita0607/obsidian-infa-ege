...
## 2 задача
![[Pasted image 20230304143831.png]]
...
```
## 3 задача
![[Pasted image 20230304144538.png]]
...
## 1 задача
==mega easy==
![[Pasted image 20230303072812.png]]
```python
file = open("27-1a.txt")
n = int(file.readline()) #считываем первый файл == кол-во пар
min_sum = 0
min_raznitza_ne_del_na_3 = float("inf") #наша задача – найти сумму, не делящуюся на 3, следовательно, если в конце концов у нас получится сумма, делящаяся на 3, мы просто добавим эту разницу к нашей сумме
...
![[Pasted image 20230304124208.png]]
...
if sum_ab%3 == 0:  #Если сумма делится как надо – тогда выводим
    ...
    if sum_ab%3 == 1:  #Убираем остаток, который в остатке от деления на 3 даёт 1
        ...
        sum_ab-=min_r_2#Убираем остаток, который в остатке от деления на 3 даёт 2
        print(sum_ab)
```
## 3 задача
==pretty easy==
![[Pasted image 20230304131314.png]]
...
![[Pasted image 20230304135337.png]]
...
```python
file = open("27-8a.txt")  
n = int(file.readline()) #считываем кол-во элементов
min_sum_sqrt = float("inf") #создаём переменную для минимальной суммы квадратов
s = [int(x) for x in file] #считываем в массив все числа из файла
...
for i in range(n-5):  
    ...
    mass[0] = min(mass[0], mass[1]) #нулевой особенный  
    for y in range(3):  
        mass[y+1] = mass[y+2] # продвигаем элементы вверх, не считая 0(1) элемент и 4(5)  
    mass[4] = x #добавляем в массив [второй элемент]
