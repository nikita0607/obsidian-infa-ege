...
s = s.replace('b', 'c') # s = 'acca'
s = s.replace('c', 'a') # s = 'aaaa'

# Это возвращает новую строчку!
# а не изменяет исходную!
s.replace('a', 'b') # s = 'aaaa'

# Заменить первые n символов начтная слева строки
s = s.replace('a', '0', 1) # s = '0aaa'
s = s.replace('a', 'b', 2) # s = '0bba'
...
##### Поиск делителей
```python
# Например, найдем все делители числа 125
n = 125

dels = []
for de in range(1, int(n**0.5)+1): # Делаем перебор до (кв. корня + 1)
	if n%de==0: # Если число нацело делиться
		'''
		Если число n представить в виде произв. двух чисел, то получится:
		n = x*y, где x = `de`, а y = `n//de`
		'''
		dels.append(de) # Запоминаейм первый делитель
		if de!=n//de: # Проверяем что делитель – не кв. корень из числа
			dels.append(n//de) # Добавляем второй делитель

'''
То есть, если проверяемый делитель является квадратным корнем, то второй делитель будет так же квадратным корнем, а значит нам нужно сохранить только один из двух делителей
'''
```
...
Размер задан пикселями, так что количество пикселей = перемножаем одно на другое
L = 960 * 512 = 491 520
...
Очень простая задача. Всё что нам нужно - это написать алгоритм из задачи на питончике:
```python
s = '9'*84
...
А теперь попробуем написать программу, которая пойдет от конечной строки (необходимого нам результата = `'1'`) до начальной (сост. из `3`):
```python
s = "1"
...
Для решения этой задачи просто переберем 
```python
arr = []
...
## Задачи на маски
Тут нам прегодится модуль `fnmatch`
...
Например, маске 123 * 4?5 соответствуют числа 123405 и 12300405. Среди натуральных чисел, не превышающих 10^8, найдите все числа, которые делятся на сумму нечётных цифр числа и соответствующие маске 124 * 5 * 79. В ответе запишите в первом столбце таблицы все найденные числа в порядке возрастания, а во втором столбце – сумму всех цифр этого числа.
___

Итак, немного поразмышляем
    Маска всегда заканчивается на 79. Значит, можно начать перебор с самого маленького числа, соответствующего маске и брать числа с шагом `100`, что бы две последние цифры оставались на месте

```python
from fnmatch import fnmatch

nums = []
for n in range(124579, 10**8, 100):
    if fnmatch(str(n), "124*5*79"): # Если число соотв. маске
        s = 0
        for sym in str(n):  # Считаем сумму нечетных цифр числа
            if int(sym) % 2 == 1:
                s += int(sym)

        if n % s == 0: # Если делится нацело
            nums.append(n)

for n in nums:
    print(n, sum(map(int, str(n)))) # Сумма цифр числа в одну строку
```
`Ответ:` 1249579 37
            12409579 37
            12452979 39
