...
def f(k1, k2, s, m):
	...
	# Если игрок победил
	if k1+k2 >= MAX_VAL:
		return s%2 == m%2 # Cовершил ли последний ход игрок, который нам нужен
	...
	moves = [f(k1+1, k2, s+1, m), f(k1*2, k2, s+1, m),
			 f(k1, k2+1, s+1, m), f(k1, k2*2, s+1, m)]
...
for s in range(1, 61):
	...
	for m in range(1, 5):
		...
		if a == 1 and f(12, s, 0, m):
			break
		# А если третьим, то это наш ответ
		if a == 3 and f(12, s, 0, m):
			print(s)
```
